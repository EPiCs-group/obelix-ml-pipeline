...
# objective 1a: For a complete new substrate, the model gives the performance of 192 ligands with an accuracy as high as possible
# in this case the ligands that are active for all training substrates are removed from the dataset
from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor

from obelix_ml_pipeline.load_representations import prepare_selected_representations_df
from obelix_ml_pipeline.machine_learning import prepare_classification_df, train_ml_model, predict_ml_model, reduce_dimensionality_train_test, filter_ligands_with_only_one_class
...
def predict_out_of_sample_substrate(selected_ligand_representations, selected_substrate_representations,
    ...
    if 'accuracy' in scoring:  # this means that we are doing a classification task
        # filtering is done such that ligands that are active for all training substrates are removed from the dataset
        ligands_to_remove = filter_ligands_with_only_one_class(train_data, target, target_threshold, binary, ligand_numbers_column, list_of_training_substrates, print_ml_results)
        # remove these ligands from the training data
        train_data = train_data[~train_data[ligand_numbers_column].isin(ligands_to_remove)]
        train_data = prepare_classification_df(train_data, target, target_threshold, binary)
        # same for test data
        test_data = test_data[~test_data[ligand_numbers_column].isin(ligands_to_remove)]
