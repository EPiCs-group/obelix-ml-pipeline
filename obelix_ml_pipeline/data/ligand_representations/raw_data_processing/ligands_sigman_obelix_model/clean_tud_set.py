...
if __name__ == "__main__":
    ...
    df.to_csv('sigman_structures_dft_SP_obelix_descriptors.csv', index=False)

    # select only the 70 structures for which we did not swap descriptors (i.e. the structures that are directly comparable)
    ligand_info_jnj = pd.read_excel('JNJ_Ligands_Sigman_descriptors.xlsx', sheet_name='Ligand')
    descriptors_df = pd.read_csv('sigman_structures_dft_SP_obelix_descriptors.csv')
    list_of_overlapping_ligands_without_swapping = ligand_info_jnj.loc[
        ligand_info_jnj['SWAPPED (1=yes, 0=no, #N/A=no overlap)'] == 0, 'Ligand#'].tolist()

    # select only the ligands that are in common and not swapped (since these structures are directly comparable)
    ligand_descriptors_obelix_merged = descriptors_df.loc[descriptors_df['Ligand#'].isin(list_of_overlapping_ligands_without_swapping)]

    ligand_descriptors_obelix_merged.to_csv('ligands_sigman_obelix_model.csv', index=False)
